...
if __name__ == '__main__':
    str_l = "这是一个test字符串，fda,fdas.，。；？爾雅 郭璞，字景純。《/○□彳●=～＋「、n"
...
# file1 = open('D:\\pythonProject\\hant_a.txt', 'r', encoding='utf-8')
#
# data = file1.read()
# str = []
# for ch in data:
...
for file in files:
    ...
    f1 = open('D:\\pythonProject\\训练集_一个文件夹\\%s' % file, 'r', encoding='utf-8')
    f1_data = f1.read()
    f1.close()
    # print(f1_data)
    ...
    if f1_data[-1] != 'a':
        num2 += 1
        # print('**' + f1_data[-3:-1] + f1_data[-1])
        print(file + '**结尾错误**', num2)
        file_err.append(file)

        # f1 = open('D:\\pythonProject\\训练集_一个文件夹\\%s' % file, 'w', encoding='utf-8')
        # f1.write(f1_data)
        # f1.close()
...
# 文件夹所在路径
dir = "D:\\pythonProject\\训练集_四种标记0.1"
...
f2 = open('D:\\pythonProject\\汉字标记集\\%s' % file, 'r', encoding='utf-8')
f2_data = f2.read()
print(f2_data)
f2_list = re.findall(r'\((.*?)\)', f2_data, re.S)
print(f2_list)
print(len(f2_list))
...
# 将以string字符串形式存入文件的list 变为list
'''
f1 = open('D:\\pythonProject\\汉字标记集\\pro_hant_a0.txt', 'r', encoding='utf-8')
data = f1.read()[:-1]
print(data)
d = data.split(',')
print(d)
d1 = [d[i][3:-1] for i in range(0,len(d), 2)]

d2 = [float(d[i][1:-1]) for i in range(1,len(d), 2)]
print(d1[:50])
print(d2[:50])
dic_a = dict(zip(d1,d2))
# 将 二元组频率字典 存入文件
f2 = open('D:\\pythonProject\\汉字标记集\\a.txt', 'w', encoding='utf-8')
json.dump(dic_a, f2)
f2 = open('D:\\pythonProject\\汉字标记集\\a.txt', 'r', encoding='utf-8')
dic_a = json.load(f2)
# print('\n'.join([str(i) for i in list(dic_a.items())[:100]]))

'''


# 猴子吃桃问题：猴子第一天摘了若干个桃子，当即吃了一半，还不解馋，又多吃了一个；第二天，吃剩下的桃子的一半，还不过瘾，又多吃了一个；
# 以后每天都吃前一天剩下的一半多一个，到第n天想再吃时，只剩下一个桃子了。问第一天共摘了多少个桃子？
'''
def taozi_num(n): # n为桃子数为1时 的天数
	if n == 1:
		return 1
	else:
		return (taozi_num(n-1) + 1) * 2


if __name__ == '__main__':

	m = input('请输入到第n天再想吃时，只剩下一个桃子了，n值为：')
	# print (m)
	num = taozi_num(int(m))
	print('第一天摘的桃子个数：', num)
